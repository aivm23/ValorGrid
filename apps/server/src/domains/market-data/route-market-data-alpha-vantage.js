...
module.exports = function attach(ctx) {
  assertCtxDeps(ctx, ['sendJson', 'readJsonBody', 'config', 'readAlphaVantageKey', 'saveAlphaVantageKey', 'deleteAlphaVantageKey'], 'route-market-data-alpha-vantage');
  ...
  async function testAlphaVantageKey(key) {
    ...
    if (!fetchResponse.ok) return { valid: false, limited: false, error: `Alpha Vantage responded ${fetchResponse.status}` };
    ...
    if (payload.Information) return { valid: false, limited: payload.Information.includes('limit'), error: payload.Information };
  ...
  function secretsDir() {
    ...
    return userDataDir ? userDataDir + '/backups' : config?.backupDir || '';
  }
  ...
  async function handleAlphaVantageKeyRoutes(_ctx, request, response, url) {
    if (url.pathname === '/api/market-data/alpha-vantage/status' && request.method === 'GET') {
      ...
      sendJson(response, 200, {
        ...
        source: key ? (isDesktopMode() ? 'local' : 'env') : null,
        hint: !key
          ? isDesktopMode()
    ...
    if (url.pathname === '/api/market-data/alpha-vantage/key' && request.method === 'POST') {
      if (!isDesktopMode()) {
        sendJson(response, 400, {
          error: 'Alpha Vantage API key solo puede guardarse localmente desde la app de escritorio',
          hint: 'En modo servidor, configura VALORGRID_ALPHA_VANTAGE_API_KEY en variables de entorno',
        });
        return true;
      }
      const body = await readJsonBody(request);
      const key = String(body.apiKey || '').trim().toUpperCase();
      ...
      try {
        ...
        if (!testResult.valid) {
          if (testResult.limited) {
            ...
            setKey(key);
            sendJson(response, 200, { message: 'Clave guardada. El límite diario de Alpha Vantage está activo (25 llamadas/día).' });
            return true;
          }
          sendJson(response, 400, { error: 'La clave no es válida. Alpha Vantage no la reconoce.', hint: testResult.error });
          return true;
        }
        saveAlphaVantageKey(secretsDir(), key);
        setKey(key);
    ...
    if (url.pathname === '/api/market-data/alpha-vantage/key' && request.method === 'DELETE') {
      if (!isDesktopMode()) {
        sendJson(response, 400, {
          error: 'Esta acción solo está disponible en la app de escritorio',
      ...
      clearKey();
      sendJson(response, 200, { message: 'Clave de Alpha Vantage eliminada. Los precios de commodities quedan desactivados.' });
      return true;
    }
