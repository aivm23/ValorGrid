...
const { createAlphaVantageMarketData } = require('./market-data-alpha-vantage');
const { parseDailyPrices } = require('./market-data-yahoo-helpers');
...
module.exports = function attach(ctx) {
  ...
  const { fetchAlphaSpotPrice, fetchAlphaDailyPrices } = createAlphaVantageMarketData({
    getMemoryCached,
    setMemoryCached,
    upsertMarketPricePoint,
    upsertProviderState,
  ...
  function getBestLocalQuote(yahooSymbol, requestedDate = getToday()) {
    return (
      ...
      normalizeLocalQuote(
        getLatestMaterializedPrice(yahooSymbol, requestedDate),
        requestedDate,
        'materialized-cache',
      ) ||
  ...
  function firstDailyCloseAtOrAfter(result, requestedDate) {
    ...
    for (let index = 0; index < timestamps.length; index += 1) {
      ...
      const closeDayTime = Date.UTC(closeDate.getUTCFullYear(), closeDate.getUTCMonth(), closeDate.getUTCDate());
  ...
  async function scanSplitsForYahooInstrument(instrument, fromDate = null, toDate = null) {
    ...
    try {
      ...
      /* split scans are advisory for market-data reads */
    }
  }
  ...
  function makeAlphaQuote(quote, date, source) {
    return {
      ...
      stale: quote.date !== date,
      cached: false,
      dataQuality: quote.date === date ? 'ok' : 'stale',
      priceAgeDays: daysBetween(quote.date, date),
    };
  }
  ...
  async function quoteFromSource(instrument, source, requestedDate, options = {}) {
    ...
    if (source.provider === 'alpha_vantage') {
      const cached = quoteFromMarketPoint(
        getLatestMarketPricePoint(instrument.symbol, source.provider, source.providerSymbol, date),
        date,
        source,
        instrument,
        daysBetween,
  ...
  async function getQuoteForSymbol(inputSymbol, requestedDate = null, options = {}) {
    ...
    for (const source of sources) {
      try {
        quote = await quoteFromSource(
          instrument || { symbol: normalizeSymbol(inputSymbol), yahoo_symbol: yahooSymbol },
          source,
          requestedDate,
          options,
        );
  ...
  async function getFxToEur(currencyInput, requestedDate = null, options = {}) {
    const currency = String(currencyInput || 'EUR')
      .trim()
      .toUpperCase();
  ...
  Object.assign(ctx, {
    ...
    fetchDatedYahooPriceWithFallback,
    fetchLatestYahooPriceWithFallback,
    getBestLocalQuote,
    resolvePriceSourcesForInstrument,
    dailyCacheHasRange,
