...
module.exports = function attach(ctx) {
  ...
  const { buildMonthlyGroups, topMonthlyGroup } = createPortfolioMonthlyHelpers({
  ...
  async function scanCorporateActionsQuietly(toDate = getToday()) {
    if (typeof ctx.scanCorporateActions === 'function') await ctx.scanCorporateActions({ toDate }).catch(() => {});
  }
  ...
  async function buildSummary() {
    ...
    if (groupsEnabled) {
      ...
      portfolio = visibleValuations
        .filter((i) => i.showInDistribution)
        .map((i) => {
  ...
  function isEffectiveValuation(item) {
    return isEffectiveValuationWithMinimum(item, minimumDisplayValueEur);
  }
  ...
  async function buildMonthly(year) {
      ...
      try {
        ...
        try {
          ...
          valuations = await Promise.all(
            instruments.map((item) =>
              getInstrumentValuationAt(dbInstrument(item.symbol), fallbackRequestedDate, asOfDate),
            ),
          );
        }
        const cells = {};
        for (const column of configuredColumns) {
          const positions = valuations.filter(
            (item) =>
              (column.isGroup ? item.groupId === column.id : item.symbol === column.id) && isEffectiveValuation(item),
          );
          if (!positions.length) {
            cells[column.id] = {
              value: null,
        ...
        monthlyInsights.push({
          ...
          autoStatus:
            monthFlows.autoContributions > 0 ? `${monthFlows.autoContributions} automáticas` : 'Sin automáticas',
          groups: monthGroups,
        });
    ...
    const completedMonths = monthlyInsights.filter(
      (month) => month.total !== null && Number.isFinite(Number(month.total)),
    );
