...
module.exports = function attach(ctx) {
  ...
  const translate = (request, message) =>
    typeof ctx.translateForRequest === 'function' ? ctx.translateForRequest(request, message) : message;
  ...
  const server = http.createServer(async (request, response) => {
    ...
    try {
      ...
      sendJson(response, 400, { error: translate(request, 'Bad request') });
      return;
    }
    ...
    if (url.pathname.startsWith('/api/')) {
      try {
        ...
        if (!handled) sendJson(response, 404, { error: translate(request, 'Not found') });
      } catch (error) {
        console.error(error);
    ...
    if (extensionAssetPath) {
      try {
        ...
        response.end(translate(request, 'Not found'));
      }
      return;
    }
    ...
    if (!filePath) {
      ...
      response.end(translate(request, 'Forbidden'));
      return;
    }
