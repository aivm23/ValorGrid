...
import { createImportBatchManager } from './import-batches.js';
import {
  toBase64,
  isXlsxSource,
  resetImportDraft,
...
export function attach(ctx) {
  ...
  async function advanceImportStep() {
    ...
    if (current === 'instruments') {
      ...
      if (detailsBefore.length) {
        ...
        ctx.elements.importFeedback.textContent = ctx.t('import.feedback.incompleteInstruments', {
          details: parts.join('; '),
        });
  ...
  function renderImportPreview() {
    ...
    ctx.elements.importMappingRequired.hidden = true;
    ctx.elements.importMappingRequired.innerHTML = '';
    ctx.elements.importPreviewOutput.innerHTML = renderImportPreviewContent(
      ctx,
      preview,
      ctx.state,
      preview.warnings || [],
  ...
  async function previewCsvImport(options = {}) {
    ...
    try {
      ...
      ctx.elements.importPreview.disabled = Boolean(
        ctx.state.importWorkflowBusy || !ctx.state.importFileMeta || ctx.state.importPreview,
      );
    }
  }

  async function commitCsvImport() {
    if (!ctx.state.importPreview?.canCommit) {
      ctx.elements.importFeedback.textContent = 'No se puede importar: hay filas pendientes de resolución.';
      return;
    }
  ...
  function handleImportPreviewInteraction(event) {
    ...
    if (rowSymbol) {
      ...
      const value = String(rowSymbol.value || '')
        .trim()
        .toUpperCase();
      if (value) {
    ...
    if (choiceSymbol) {
      ...
      if (ctx.state.importInstrumentChoices[key])
        ctx.state.importInstrumentChoices[key].symbol = String(choiceSymbol.value || '')
          .trim()
          .toUpperCase();
      invalidateImportAfter(ctx, 'instruments');
      return;
    ...
    for (const fieldName of fields) {
      ...
      const normalized = ['Symbol', 'Currency'].includes(fieldName)
        ? String(node.value || '')
            .trim()
            .toUpperCase()
        : node.value;
      updateChoiceCreateField(
        key,
        fieldName === 'Symbol' ? 'symbol' : fieldName === 'Yahoo' ? 'yahooSymbol' : fieldName.toLowerCase(),
        normalized,
      );
