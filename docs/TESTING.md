...
| `test/benchmark.test.js`           | Rendimiento   | Contabilidad recursiva de recursos estáticos y contrato de muestreo frío/caliente del benchmark local reproducible.                                                                                                                              |
...
| `test/db-operations.test.js`       | DB Ops        | Resolución de path activo, backup manual y automático verificado, aborto seguro del reset, recreación fresh, doctor, sincronía `DATA_MODEL.md` -> `schema.js`, prohibición `ALTER TABLE` y canonicalidad de `seed:demo`.                         |
| `test/db-migrations.test.js`       | Migraciones   | Sistema de migraciones automáticas: DB vacía no migra, backup previo, actualización de `schema_version`, inferencia por columnas, idempotencia, error detiene arranque, modo Docker deshabilitado por defecto.                                   |
...
| `test/financial-semantics.test.js` | Cálculos      | `buildPortfolioPerformance`, FIFO realized gain, splits/reverse splits sin cash-flow, signos de cash-flow, fórmulas YTD y `history.series[].contributed`.                                                                                        |
...
| `test/update-service.test.js`      | Actualización | Servicio de actualización: comparación semver, selección de asset desktop por plataforma/arquitectura, detección de runtime, imagen GHCR esperada y endpoint `/api/update/status` con fallback si GitHub falla.                                  |
...
| `test/portfolio.test.js`           | Core CRUD     | Transacciones, dividendos desde Yahoo, splits Yahoo idempotentes, instrumentos, grupos, identifiers, fuentes/precios de mercado, auto-plans, backups, health, onboarding, state, quote, export XLSX y endpoints legacy 404.                      |
| `test/portfolio-history.test.js`   | Histórico     | Materialización daily/weekly, posiciones ajustadas por split, invalidación, cache persistente, restart survival, daily prices/FX, summary, monthly, rangos y dataset demo/loadtest.                                                              |
