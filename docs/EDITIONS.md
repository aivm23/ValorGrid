...
Professional ofrece conectores de importación y capacidades comerciales adicionales bajo condiciones propietarias. Su código, documentación operativa, pruebas y datos de ejemplo viven fuera de este repositorio público.

Si Professional necesita compatibilidad temporal de schema con una versión Community concreta, esa capa pertenece a la distribución no pública y debe ser retirada o formalizada allí. Community mantiene como contrato público `apps/server/src/schema.js` para instalaciones limpias y los SQL versionados de `deploy/sql/` para actualizaciones explícitas.
...
- `apps/server/src/schema.js` es la fuente canónica del schema Community para instalaciones limpias. Ninguna edición puede modificar este contrato público sin pasar por el repositorio Community. Los SQL versionados de `deploy/sql/` son el mecanismo de actualización explícita para bases existentes; las ediciones privadas no pueden introducir migraciones runtime ni alterar la política fresh-only de Community.
