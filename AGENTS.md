...
- Lint/format/typecheck/build are introduced incrementally by migration phase, never as a big-bang rewrite.
...
- Migration checkpoints run in this order:
  ...
  2. full `npm test`,
  3. `npm run verify:publication`,
  4. docs sync verification when docs/API/schema/module lists change.
